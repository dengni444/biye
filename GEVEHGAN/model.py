        ...
        if isinstance(inputs, tuple) or g.is_block:
            ...
            for stype, etype, dtype in g.canonical_etypes:
                ...
                dstdata = self.mods[etype](
                    ...
                    *mod_args.get(etype, ()),
                    **mod_kwargs.get(etype, {}))
                outputs[dtype].append(dstdata)         #传入子图 使用层级注意力 得到节点特征
        rsts = {}
        for nty, alist in outputs.items():            #节点之间连了多种类型边
            if len(alist) != 0:
                if nty == 'user':
...
#             rel: Edge_level(hid_feats * num_heads, out_feats, num_heads=num_heads, )
#             for rel in rel_names}, in_size_sem=out_feats, num_head=num_heads)
#         self.conv3 = HeteroGraph({
#             rel: Edge_level(out_feats * num_heads, out_feats, num_heads=num_heads, )
#             for rel in rel_names}, in_size_sem=out_feats, num_head=num_heads)
#         self.lin = nn.Linear(hid_feats*num_heads+out_feats * num_heads+out_feats * num_heads, out_feats)
#         # self.lin = nn.Linear(out_feats * num_heads, out_feats)
#         self.lin2 = nn.Linear(out_feats, out_feats)
#         self.relu = nn.ReLU()
#
...
#     def forward(self, graph, inputs, edge_attr):
#         # 第一层卷积
#         h1 = self.conv1(graph, inputs, edge_attr)
#         h1 = {k: v.reshape(v.shape[0], -1) for k, v in h1.items()}
#         h1 = {k: self.relu(v) for k, v in h1.items()}
#
#         # 第二层卷积
#         h2 = self.conv2(graph, h1, edge_attr)
#         h2 = {k: v.reshape(v.shape[0], -1) for k, v in h2.items()}
#         h2 = {k: self.relu(v) for k, v in h2.items()}
#
#         # 第三层卷积（多尺度）
#         h3 = self.conv3(graph, h2, edge_attr)
#         h3 = {k: v.reshape(v.shape[0], -1) for k, v in h3.items()}
#         h3 = {k: F.relu(v) for k, v in h3.items()}
#
#         # 融合三个尺度的特征（拼接）
#         h = {k: torch.cat([h1[k], h2[k], h3[k]], dim=-1) for k in h1.keys()}  # 拼接特征
#         # 通过线性层处理
#         h = {k: self.lin(v) for k, v in h.items()}
#         h = {k: self.lin2(v) for k, v in h.items()}
#         return h
...
class VAE(nn.Module):
    ...
    def decode(self, z):
        ...
        x_recon = torch.sigmoid(self.fc_out(h))  # 根据数据分布选择激活函数
        return x_recon
...
class Model(nn.Module):
    def __init__(self, in_features, hidden_features, out_features, rel_names, num_heads,latent_dim=64,num_scales=3):
        super().__init__()
        self.rel_names = rel_names

        self.sage = HMGNN(in_features, hidden_features, out_features, rel_names, num_heads)
        # self.sage = HMGNN_WithTransformer(in_features, hidden_features, out_features, rel_names, num_heads,
        ...
        # VAE 部分
        self.vae_user = VAE(in_features, hidden_features, latent_dim)
        self.vae_poi = VAE(in_features, hidden_features, latent_dim)

        # 更新节点特征维度
        self.fc_list_node = nn.ModuleList([
            nn.Linear(latent_dim, in_features, bias=True) for _ in range(2)  # 假设有两个节点类型：'user' 和 'poi'
        ])
        ...
        self.fc_list_edg = nn.ModuleList(
            [nn.Linear(feats_dim, 5, bias=True) for feats_dim in [5, 4 ,2]])
    ...
    #
    #     # 调试输出：打印节点类型和是否存在于h中
    #     print("Checking if all nodes are in h:")
    #     for idx in edge_index[0]:
    #         if idx.item() not in h['user']:  # 假设你正在访问 'user' 类型的节点
    #             print(f"Node {idx.item()} not found in h['user']")
    #     for idx in edge_index[1]:
    #         if idx.item() not in h['user']:
    #             print(f"Node {idx.item()} not found in h['user']1")
    #
    #     logits = cosine_similarity(h['user'][edge_index[0]], h['user'][edge_index[1]])
    #     logits_2 = self.relu(logits)
    #     return logits_2
    ...
    def forward(self, g, neg_g, node_feat, edge_attr, etype):
        ...
        # 获取用户和POI特征
        user_feat = node_feat['user']
        poi_feat = node_feat['poi']

        # print(user_feat)
        # print(poi_feat)
        #
        # print(user_feat.requires_grad)  # 输出True说明启用了梯度计算
        # print(poi_feat.requires_grad)  # 输出True说明启用了梯度计算


        # 对用户节点特征进行 VAE 编码
        user_recon, user_mu, user_logvar = self.vae_user(user_feat)
        # 对 POI 节点特征进行 VAE 编码
        poi_recon, poi_mu, poi_logvar = self.vae_poi(poi_feat)

        # 获取潜在表示
        user_z = self.vae_user.reparameterize(user_mu, user_logvar)
        poi_z = self.vae_poi.reparameterize(poi_mu, poi_logvar)

        # 更新节点特征
        feat2 = {}
        feat2['user'] = F.relu(self.fc_list_node[0](user_z))
        feat2['poi'] = F.relu(self.fc_list_node[1](poi_z))
        ...
        # VAE 的重构损失和 KL 散度
        recon_loss_user = F.mse_loss(user_recon, user_feat)
        recon_loss_poi = F.mse_loss(poi_recon, poi_feat)
        kl_loss_user = -0.5 * torch.mean(1 + user_logvar - user_mu.pow(2) - user_logvar.exp())
        kl_loss_poi = -0.5 * torch.mean(1 + poi_logvar - poi_mu.pow(2) - poi_logvar.exp())
        # beta = 0.2
        # vae_loss = (recon_loss_user + recon_loss_poi)*beta + (kl_loss_user + kl_loss_poi)*0.2
        beta = 0.2
        vae_loss = (recon_loss_user + recon_loss_poi)*beta + (kl_loss_user + kl_loss_poi)*0.2

        return self.pred(g, h, etype), self.pred(neg_g, h, etype), h, contrastive_loss(h['user'], g),vae_loss
