...
2B. 聚类质量指标对比 - 固有结构分析 (k=2-15) [去掉DB指数]
3. 范数分布对比 - 层级编码能力

使用方法:
    python paper_figures_three_comparisons_cn_notitle_no_db.py --city NYC
    python paper_figures_three_comparisons_cn_notitle_no_db.py --city SP
"""
...
metrics_euc = {'Silhouette': [], 'Calinski-Harabasz': []}
metrics_hyp = {'Silhouette': [], 'Calinski-Harabasz': []}
...
# ========== 图2B: 聚类质量指标对比 (k=2-15，折线图) - 不包含DB指数 ==========
print("\n📈 生成图2B: 聚类质量指标对比 (不含DB指数)...")
...
ax.set_title('(b) Calinski-Harabasz分数', fontsize=11, fontweight='bold')
ax.set_xticks(k_range)
ax.legend(fontsize=10, loc='best')
ax.grid(True, alpha=0.3, linestyle='--')
